...
class DocumentTypeService extends BaseService {
    ...
    async getDocumentTypeByTransactionId(transactionId) {
        try {
            ...
            if (!transInfo) throw MyError.badRequest(`${this.tableName} Service`, "Transaction is not found!");

            if (!this.currentUser.isAdmin && transInfo.userId !== this.currentUser.id) throw MyError.badRequest(`${this.tableName} Service`, "Transaction is not belongs to you!");
    ...
    async getById(documentTypeId) {
        try {
            ...
            // Check user is exist.
            if (!documentType) throw MyError.badRequest(`${this.tableName} Service`, "Document Type is not found!");
            return documentType;
        } catch (err) {
            if (err instanceof MyError.MyError) throw err;
    ...
    async update(documentTypeId, data, loggingDb) {
        try {
            if (!this.currentUser.isAdmin) throw MyError.forbidden(`${this.tableName} Service`);
            const fields = ['id', 'name','isRequired', 'isBoth', 'isListing', 'createdAt', 'updatedAt'];
            const documentTypeExist = await this.repository.getById(documentTypeId, fields);
            if (!documentTypeExist) throw MyError.badRequest(`${this.tableName} Service`, "Document Type is not found!");
            // Update
            const result = await this.repository.updateItem(data, { id: documentTypeId });
            // Get data updated
            const updated = await this.repository.getById(documentTypeId, fields);
            // Loging DB Update
            loggingDb.updateItem(this.currentUser.id, this.tableName, documentTypeExist, updated);
    ...
    async delete(documentTypeId, loggingDb) {
        try {
            if (!this.currentUser.isAdmin) throw MyError.forbidden(`${this.tableName} Service`);
            const [documentTypeExist, documentTypeIsUsed] = await Promise.all([
                this.repository.getById(documentTypeId, ['id', 'name','isRequired', 'isBoth', 'isListing', 'createdAt', 'updatedAt']),
                this.repository.getDocumentTypeIsUsed(documentTypeId)
            ])
            if (!documentTypeExist || documentTypeIsUsed) throw MyError.badRequest(`${this.tableName} Service`, "Document Type is not found or is used by transaction!");
            // Delete
            await this.repository.deleteItem({ id: documentTypeId });
            // Loging DB Delete
            loggingDb.deleteHardItem(this.currentUser.id, this.tableName, documentTypeExist);
            return {
                id: documentTypeId
            }
        } catch (err) {
