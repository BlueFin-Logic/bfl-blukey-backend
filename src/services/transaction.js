...
class TransactionService extends BaseService {
    ...
    async getAll(page, limit, query) {
        try {
            ...
            if (query.startDate || query.endDate) {
                const startDate = query.startDate ? Time.formatTimeToStringDate(query.startDate) : null;
                const endDate = query.endDate ? Time.formatTimeToStringDate(query.endDate) : null;
                if (startDate) {
            ...
            const fields = [
                'id', 'address', 'city', 'state', 'zipCode', 'mlsId', 'apn', 'listingPrice', 'commissionAmount', 'buyerName', 'sellerName', 'isListing', 'listingStartDate', 'listingEndDate', 'createdAt', 'updatedAt',
            ];
            const include = [
                {
    ...
    async getById(transId) {
        try {
            ...
            if (!transaction) throw MyError.badRequest(`${this.tableName} Service`, "Transaction is not found!");
            return transaction;
        } catch (err) {
            if (err instanceof MyError.MyError) throw err;
    ...
    async suggest(query) {
        try {
            ...
            if (!buyerName && !sellerName) throw MyError.badRequest(`${this.tableName} Service`, "Record is not found!");
            ...
            // Check transaction is exist.
            if (!transaction) throw MyError.badRequest(`${this.tableName} Service`, "Record is not found!");
            return transaction;
        } catch (err) {
            if (err instanceof MyError.MyError) throw err;
            throw MyError.cannotGetEntity(`${this.tableName} Service`, this.tableName, err);
    ...
    async create(transaction, loggingDb) {
        try {
    ...
    async update(transId, data, loggingDb) {
        try {
            const fields = [
                'id',
                'userId',
            ...
            const transactionExist = await this.repository.getById(transId, fields);
            // Check transaction is exist.
            if (!transactionExist) throw MyError.badRequest(`${this.tableName} Service`, "Transaction is not found!");
            // Check transaction belongs to User.
            if (transactionExist.userId !== this.currentUser.id) throw MyError.badRequest(`${this.tableName} Service`, "Transaction is not belong to you!");
            // Only permission update when status is NEW(1) or IN PROCESS(2)
            if (transactionExist.transactionStatusId !== 1 && transactionExist.transactionStatusId !== 2) throw MyError.badRequest(`${this.tableName} Service`, "Transaction can not update because status is not NEW or IN PROCESS!");

            // Update
            const result = await this.repository.updateItem(data, { id: transId });
            // Get data updated
            const updated = await this.repository.getById(transId, fields);
            // Loging DB Update
            loggingDb.updateItem(this.currentUser.id, this.tableName, transactionExist, updated);
    ...
    async status(transId, status, emailService, loggingDb) {
        try {
            const fields = ['id', 'userId', 'transactionStatusId', 'address', 'city', 'state', 'zipCode', 'mlsId', 'apn', 'listingPrice', 'commissionAmount', 'buyerName', 'sellerName', 'isListing', 'listingStartDate', 'listingEndDate', 'createdAt', 'updatedAt'];
            const transaction = await this.repository.getById(transId, fields);
            // Check transaction is exist.
            if (!transaction) throw MyError.badRequest(`${this.tableName} Service`, "Transaction is not found!");
            ...
            if (status === 2
                ...
                const updated = await this.repository.getById(transId, fields);
                // Loging DB Update
                loggingDb.updateItem(this.currentUser.id, this.tableName, transaction, updated);
            ...
            if (status === 3
                ...
                if (rest.length > 0) throw MyError.badRequest(`${this.tableName} Service`, "Need upload all required documents before change status to review!");
            ...
            throw MyError.badRequest(`${this.tableName} Service`, `Cannot update status for transaction Id: ${transId}`);
        } catch (err) {
            if (err instanceof MyError.MyError) throw err;
            throw MyError.cannotUpdateEntity(`${this.tableName} Service`, this.tableName, err);
        }
